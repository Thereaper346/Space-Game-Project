// ============================================================================
// FILE: main.js (Master File - Fully Patched & Consolidated)
// ============================================================================
...
let dungeonSys = new DungeonManager(); 
...
function setupUI() {
    ...
    bindClick('btn-continue', () => {
        try {
            ...
            console.error("[SCAVENGER OS ERROR] Save file corrupted!", err);
    ...
    bindClick('btn-tool-eraser-draw', () => setTool('ERASER', 'btn-tool-eraser-draw'));
    bindClick('btn-tool-line', () => setTool('LINE', 'btn-tool-line'));
    bindClick('btn-tool-rect', () => setTool('RECT', 'btn-tool-rect'));
    ...
    const filters = [
        ...
        { id: 'filter-dungeons', target: 'tab-world-part' },
        { id: 'filter-items', target: 'tab-part' }
    ];
    filters.forEach(f => {
        bindClick(f.id, () => {
    ...
    bindClick('btn-action-load', () => {
        if (globalSelectedIndex !== null) {
            ...
            editor.loadPartForEdit(globalSelectedIndex);
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (loadedData.library) {
                        importedParts = loadedData.library;
                        // Only load the grid workspaces if this is the FIRST file you are loading
                        // This prevents a new file from wiping out the ship you are currently drawing!
                        if (loadedData.workspaces && editor.partsLibrary.length === 0) {
                            editor.partGrid = loadedData.workspaces.partGrid || editor.createEmptyGrid(16); 
                            editor.worldPartGrid = loadedData.workspaces.worldPartGrid || editor.createEmptyGrid(32);
                            editor.shipGrid = loadedData.workspaces.shipGrid || editor.createEmptyAssembly(11); 
                            editor.worldAssemblyGrid = loadedData.workspaces.worldAssemblyGrid || editor.createEmptyAssembly(11);
                        }
                    } else { 
                        importedParts = loadedData; 
                    }
                    
                    // THE MAGIC FIX: Spread the new parts and push them into the existing array
                    editor.partsLibrary.push(...importedParts);
                    
                    updateLibraryUI(); 
                    alert(`Successfully imported ${importedParts.length} blueprints!`);
                } catch (err) { 
                    alert("Error loading file. Is it corrupted?"); 
                }
            };
            reader.readAsText(file);
            
            // Clear the input so you can click "Load" and pick the exact same file again if you wanted to
            e.target.value = ""; 
        };
    }
...
function updateLibraryUI() {
    ...
    editor.partsLibrary.forEach((part, index) => {
        let isPart = !part.format || part.format === 'part';
        let isShip = part.format === 'ship_blueprint';
        let isStation = part.format === 'world_blueprint';
        let isDungeon = part.format === 'world_part' && part.props && part.props.type === 'maze_room';
        
        let isShipPart = (isPart || !part.format) && (!part.props || ['hull', 'engine', 'weapon'].includes(part.props.type));
        let isItemPart = part.props && ['item', 'resource'].includes(part.props.type);
        
        if (activeLibraryFilter === 'PARTS' && !isShipPart) return;
        if (activeLibraryFilter === 'SHIPS' && !isShip) return;
        if (activeLibraryFilter === 'STATIONS' && !isStation) return;
        if (activeLibraryFilter === 'DUNGEONS' && !isDungeon) return;
        if (activeLibraryFilter === 'ITEMS' && !isItemPart) return;
        ...
        let tag = '🛡️';
        if (part.props && part.props.type === 'maze_room') tag = '🏛️'; 
        else if (part.format === 'world_part') tag = '🌍'; 
        else if (part.format === 'ship_blueprint') tag = '🚀'; 
        else if (part.format === 'world_blueprint') tag = '🪐'; 
        else if (part.props && (part.props.type === 'item' || part.props.type === 'resource')) tag = '🎁'; 
...
window.addEventListener('mouseup', () => { 
    if (gameState === 'EDITOR' && isDrawing) {
        editor.paint(screenMouseX, screenMouseY, canvas, false, true); 
    }
    isDrawing = false; 
    isMouseDown = false; 
    activeShopBtnId = null; 
});
...
function drawPlayerShip() {
    ...
    try {
        ...
        console.error("[SCAVENGER OS ERROR] RENDER FAILURE: Could not draw Player Ship. Data corrupted.", error, ship);
    }
    ctx.restore();
}

function drawHUD(currentBiome, enemies, items, worldManager, stations, dungeons) { // NEW: Added dungeons parameter
    ...
    try {
        ...
        console.error("[SCAVENGER OS ERROR] RADAR RENDER FAILURE", error);
    }
    ...
    // NEW: Draw Dungeons on Radar in GOLD
    ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 5;
    if (dungeons) {
        for (let d of dungeons) {
            if (d.rooms.length > 0) {
                let firstRoom = d.rooms[0];
                if (Math.hypot(firstRoom.worldX - player.x, firstRoom.worldY - player.y) > 8000) continue; 
                let angle = Math.atan2(firstRoom.worldY - player.y, firstRoom.worldX - player.x);
                ctx.beginPath(); ctx.arc(radarCX, radarCY, radarRadius, angle - 0.08, angle + 0.08); ctx.stroke();
            }
        }
    }
...
function drawShopScreen() {
    ...
    if (activeShopTab === 'BUY') {
        ...
        if (!hasItems) {
            ...
            ctx.fillText(`YOUR CARGO HOLD IS EMPTY.`, 100, rowY);
            rowY += spacing; 
        }
    }
    
    maxShopScroll = Math.max(0, rowY - (h - 150));

    ctx.restore(); 

    ctx.fillStyle = 'rgba(0, 136, 255, 0.5)';
    ctx.fillText('SCROLL TO VIEW MORE | PRESS [E] OR [ESC] TO UNDOCK', w / 2 - 240, h - 90);
}
